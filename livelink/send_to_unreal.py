...
def pre_encode_facial_data(facial_data: list, py_face, fps: int = 60, smooth: bool = False) -> list:
    ...
    """
    #print(f"输入facial_data长度: {len(facial_data)}, 第一帧长度: {len(facial_data[0]) if facial_data else 'None'}")
    encoded_data = []
    apply_blink_to_facial_data(facial_data, default_animation_data)
    #print("apply_blink_to_facial_data完成")

    total_duration = len(facial_data) / fps
    slow_duration = 0.3 if total_duration < 1.0 else 0.5
    if total_duration < 0.5:
        slow_duration = 0.1
    #print(f"total_duration: {total_duration}, slow_duration: {slow_duration}")

    fast_duration = 0.1
    slow_blend_frames = int(slow_duration * fps)
    #print(f"slow_blend_frames: {slow_blend_frames}")
    ...
    #print(f"fast_blend_in 帧数: {len(fast_blend_in)}, 第一帧长度: {len(fast_blend_in[0]) if fast_blend_in else 'None'}")

    slow_blend_in = generate_blend_frames(
        facial_data, slow_blend_frames, default_animation_data, fps,
        set(range(51)) - FAST_BLENDSHAPES, mode='in'
    )
    #print(f"slow_blend_in 帧数: {len(slow_blend_in)}, 第一帧长度: {len(slow_blend_in[0]) if slow_blend_in else 'None'}")

    blend_in_frames = combine_frame_streams(slow_blend_in, fast_blend_in, FAST_BLENDSHAPES)
    #print(f"blend_in_frames 帧数: {len(blend_in_frames)}, 第一帧长度: {len(blend_in_frames[0]) if blend_in_frames else 'None'}")
    ...
    #print(f"主帧范围: {main_start} 到 {main_end}")

    for frame_data in facial_data[main_start:main_end]:
    #    print(f" 长度: {len(frame_data)}")
        for i in range(51):
            if i >= len(frame_data):
                #print(f"索引越界! main_frames帧索引: {i}, 帧长度: {len(frame_data)}")
                raise IndexError(f"main_frames索引越界: {i}")
            py_face.set_blendshape(FaceBlendShape(i), frame_data[i])
        encoded_data.append(py_face.encode())

    default_animation_state['current_index'] = 0
    #print("default_animation_state重置")
    ...
    #print(f"fast_blend_out 帧数: {len(fast_blend_out)}, 第一帧长度: {len(fast_blend_out[0]) if fast_blend_out else 'None'}")
    ...
    #print(f"slow_blend_out 帧数: {len(slow_blend_out)}, 第一帧长度: {len(slow_blend_out[0]) if slow_blend_out else 'None'}")

    blend_out_frames = combine_frame_streams(slow_blend_out, fast_blend_out, FAST_BLENDSHAPES)
    #print(f"blend_out_frames 帧数: {len(blend_out_frames)}, 第一帧长度: {len(blend_out_frames[0]) if blend_out_frames else 'None'}")
...
#        print(f"处理blend_out帧, 长度: {len(frame)}")
    ...
    #print(f"编码完成, encoded_data长度: {len(encoded_data)}")
    return encoded_data
